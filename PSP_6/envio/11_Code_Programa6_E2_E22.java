...
/* Date:                20/05/22                             */
/* Description:         Obtiene el RFC de las clases de un programa  */
/*************************************************************/
		...
		while(opcion != 2){	// ciclo con el cual no se acaba el programa hasta que el usuario asi lo quiera
			...
			if(opcion == 1){	// Determina si es una opcion valida
				...
				if(lineas != null){ //verifica que si encontro algo
					RFC.RFCArchivo(lineas);	// Llama al metodo que evalua el archivo
				}
			}
		}
	}
...
class Clase{
	...
	/* constructor */
	/******************/
	public Clase(String nombreClase, ArrayList<String> metodos){
...
/***************************/
/* Clase que calcula el rfc de las clases de un programa */
/***************************/
class RFC{

	/**************************/
	/* Obtiene el rfc de un archivo */
	/**************************/
	public static void RFCArchivo(ArrayList<String> lineas){
		ArrayList<Clase> elementosClases = obtenerElementosClases(lineas); //Se obtienen los elementos de las clases
		ArrayList<Integer> rClases = new ArrayList<Integer>(); //Se hace un arreglo que almacenara las clases que estan acopladas
		for(int posicion=0;posicion<elementosClases.size();posicion++){ //Se recorren los elementos de las clases
			rClases.add(RFCClase(lineas,posicion,elementosClases));// Se añade el arreglo que regresa el metodo
		}
		for(int posicion=0;posicion<elementosClases.size();posicion++){//Se recorre el arreglo de acopladas
			Salida.consola(elementosClases.get(posicion).getNombreClase()+":\n");
			int m = elementosClases.get(posicion).getMetodos().size();
			int r = rClases.get(posicion);
			Salida.consola("\tM = "+m+"\n\tR = "+r+"\n");
			int rfc = m+r;
			Salida.consola("\tRFC = "+rfc+"\n");
		}
	}

	/********************************/
	/* Obtiene el rfc de una clase */
	/********************************/
	private static int RFCClase(ArrayList<String> lineas, int posicion,ArrayList<Clase> elementosClases){
		int r = 0;
		...
		return r; //Se regresa el arreglo con true en las clases que estan acopladas
	}
	...
	private static ArrayList<Clase> obtenerElementosClases(ArrayList<String> lineas){
		...
		for(int posicion=0;posicion<lineas.size();posicion++){//Se recorren las lineas hasta encontrar una clase
			if(HerramientasClase.esClase(lineas.get(posicion))){
				...
				elementosClases.add(new Clase(nombreClase,metodos));
			}
		}
